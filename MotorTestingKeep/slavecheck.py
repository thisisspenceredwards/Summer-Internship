...
def get_stop_threads():
    #global __stop_threads
    return __stop_threads

def set_stop_threads(value):
    global __stop_threads
    __stop_threads = value
...
def set_start_time():
    ...
    __start_time = __start_time.replace(tzinfo=timezone.utc) #time thread started
    logger.debugf("set_start_time called")

__pre_altitude = None
def get_pre_altitude():
    #global __pre_altitude
    return __pre_altitude

def set_pre_altitude(start_time):
    global __pre_altitude
    __pre_altitude = get_altitude(36.9821103, -121.9722686, start_time, 0)
'''
...
class ThreadController:
    ...
    def start_thread(self, method):
        logger.debugs("start threads")
        self.set_stop_threads(False)
        self.__thread = self.get_thread()
...
class SolarThread(ThreadController):
    ...
    def trackSolar(self):
        ...
        while True:
            ...
            if difference >= Constants.ST_ANGLE:
                last_recorded_altitude = self.__moveSolar(current_altitude)
    ...
    def get_time():
        ttime = datetime.now()
        return ttime.replace(tzinfo=timezone.utc)
...
def relay_on_east():
    ...
    SolarThread.getInstance().stop_threads() #
